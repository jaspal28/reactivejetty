...
public class RestHandler {
	...
	private final Action internalServerError = (HttpServletRequest request, HttpServletResponse response) -> {
		toJsonResponse(request, response,
				new Error(500, request.getRequestURI(), request.getAttribute("internal-server-error").toString()));
	};
	...
	public final Action getEndpointIfMatches(final String path) {
		List<Endpoint> endpoints = endpointList.stream().filter(end -> urlMatch(path, end.getPath())).limit(1)
	...
	public Map<String, String> getPathVariables(final String path) {
		List<Map<String, String>> pathVariablesListMap = endpointList.stream()
				.filter(end -> urlMatch(path, end.getPath())).limit(1).map(end -> end.getPath())
	...
	private Map<String, String> extractPathVariables(String endpointPath, String requestPath) {
		...
		for (int i = 0; i < split1.length; i++) {
			...
			if (!chunck1.equals(chunck2)) {
				if (chunck2.startsWith("{") && chunck2.endsWith("}")) {
					String paramName = chunck2.replaceAll("\\{", "").replaceAll("\\}", "").replaceAll(" ", "");
	...
	protected void toJsonResponse(final HttpServletRequest request, final HttpServletResponse response,
			final Object resp) throws IOException {
		response.setContentType("application/json");
		nioResponse(request, response, JsonConverter.getInstance().getJsonOf(resp));
	}

	protected void toXmlResponse(final HttpServletRequest request, final HttpServletResponse response,
			final Object resp) throws IOException {
		response.setContentType("application/xml");
		nioResponse(request, response, XmlConverter.getInstance().getXmlOf(resp));
	}

	protected void toTextResponse(final HttpServletRequest request, final HttpServletResponse response,
			final Object resp) throws IOException {
		response.setContentType("text/plain");
		nioResponse(request, response, resp.toString());
	}

	protected void toResponse(final HttpServletRequest request, final HttpServletResponse response, final Object resp,
	...
	private void nioResponse(final HttpServletRequest request, final HttpServletResponse response, final Object resp)
			throws IOException {
				...
				while (out.isReady()) {
					if (!finalContent.hasRemaining()) {
						...
						log.info(LocalDateTime.now() + " - " + this.getClass().getSimpleName()
								+ " - close Async context from http request: " + request.getRequestURI());
						return;
					}
